...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        <img src="{if $#nvn_file_url|eq('')|not}{$#nvn_file_url}{/if}{$image.url|ezroot(no)}" width="{$image.width}" height="{$image.height}" {if $hspace}hspace="{$hspace}"{/if} style="{$inline_style}" alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
        {if $href}</a>{/if}
    {/if}
