...
{* nvNewsletter - In progress newsletter list *}

{def $newsletterCount=fetch('nvnewsletter', 'in_progress_newsletter_count')
     ...
     $groupData = ''
     $senderData = ''}
<form name="newsletters" method="post" action={'/nvnewsletter/list_in_progress/'|ezurl}>
<div class="context-block">
{* DESIGN: Header START *}<div class="box-header"><div class="box-tc"><div class="box-ml"><div class="box-mr"><div class="box-tl"><div class="box-tr">
<h1 class="context-title">{'Newsletters in progress'|i18n( 'design/nvnewsletter' )}</h1>
...
<tr>
    ...
    <th>{'Send time'|i18n( 'design/nvnewsletter' )}</th>
    <th class="tight">{'# queued'|i18n( 'design/nvnewsletter' )}</th>
    <th class="tight">{'# sent'|i18n( 'design/nvnewsletter' )}</th>
</tr>
{if $newsletterCount|eq(0)}
<tr><td>&nbsp;</td><td colspan="4">{'No newsletters in progress'|i18n( 'design/nvnewsletter' )}</td><td>&nbsp;</td></tr>
{else}
{foreach $newsletter_array as $newsletter
         sequence array( bglight, bgdark ) as $seq}
<tr class="{$seq}" {if $newsletter.status|eq(2)}style="background-color: #ff8585;"{/if}>
    <td>{if $newsletter.status|eq(2)|not}<input type="checkbox" name="NewsletterIDArray[]" value="{$newsletter.id}" title="{'Select newsletter for removal.'|i18n( 'design/nvnewsletter' )}" />{/if}</td>
    ...
    <td class="number" align="right">{if $newsletter.total_mail_count|eq(0)}{'Waiting...'|i18n('design/nvnewsletter')}{else}{$newsletter.total_mail_count}{/if}</td>
...
{include name=navigator
         ...
         page_uri='/nvnewsletter/list_in_progress'
...
<div class="block">
<input class="button" type="submit" name="MoveToDraftsButton" value="{'Return selected to drafts'|i18n( 'design/nvnewsletter' )}" title="{'Return selected to drafts.'|i18n( 'design/nvnewsletter' )}" />
</div>
