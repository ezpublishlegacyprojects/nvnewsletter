...
<div class="break"></div>
    ...
    {include uri='design:nvnewsletter_children_detailed.tpl'}
...
<div class="block">
   ...
   {/if}
    ...
    <input name="ContentLanguageCode" value="{$node.object.current_language}" type="hidden" />
    {*
    {if and( is_set( $can_create_languages[0] ), eq( $can_create_languages|count, 1 ) )}
...
{* DESIGN: Control bar END *}</div></div></div></div></div></div>
    ...
    <input type="hidden" name="RedirectURIAfterSorting" value="{concat( '/nvnewsletter/view_newsletter/', $newsletter.id )}" />
    <input type="hidden" name="RedirectURIAfterPriority" value="{concat( '/nvnewsletter/view_newsletter/', $newsletter.id )}" />
